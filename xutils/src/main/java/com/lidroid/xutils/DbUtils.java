...
public class DbUtils {
    ...
    public void update(Class<?> entityType, WhereBuilder whereBuilder, String[] updateColumnNames, String[] updateColumnValues) throws DbException {
        if (this.tableIsExist(entityType)) {
            if (updateColumnNames != null && updateColumnNames.length >= 1 && updateColumnValues != null && updateColumnValues.length >= 1 && updateColumnNames.length ==
                    updateColumnValues.length) {
                try {
                    this.beginTransaction();
                    this.execNonQuery(SqlInfoBuilder.buildUpdateSqlInfo(this, entityType, whereBuilder, updateColumnNames, updateColumnValues));
    ...
    public boolean createTableIfNotExist(Class<?> entityType) throws DbException {
        return this.createTableIfNotExist(entityType, false);
    }

    public boolean createTableIfNotExist(Class<?> entityType, boolean isReChecked) throws DbException {
        if (!this.tableIsExist(entityType, isReChecked)) {
        ...
        return this.tableIsExist(entityType, isReChecked);
    }

    public boolean tableIsExist(Class<?> entityType) throws DbException {
        return this.tableIsExist(entityType, false);
    }
    ...
    public boolean tableIsExist(Class<?> entityType, boolean isReChecked) throws DbException {
        ...
        if (table.isCheckedDatabase() && !isReChecked) {
            ...
            Cursor cursor = this.execQuery("SELECT COUNT(*) AS c FROM sqlite_master WHERE type='table' AND name='" + table.tableName + "'");
            if (cursor != null) {
                try {
                    if (cursor.moveToNext()) {
