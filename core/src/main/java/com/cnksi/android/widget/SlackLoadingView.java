package com.cnksi.android.widget;
...
/**
 * 加载动画  参考Github: https://github.com/JeasonWong
 *
 * @author lyongfly
 * @version 1.0
...
public class SlackLoadingView extends View {
     ...
     * 线条最大长度
     */
    private final int MAX_LINE_LENGTH = dp2px(getContext(), 50);
    /**
     * 线条最短长度
     */
    ...
    private int[] mColors = new int[]{0xB06ABAFE, 0xB0E6A92C, 0xB0D6014D, 0xB05ABA94};
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.AT_MOST) {
            widthSize = MAX_LINE_LENGTH;
        }
        int heightMode = MeasureSpec.getMode(heightMeasureSpec);
        int heightSize = MeasureSpec.getSize(heightMeasureSpec);
        if (heightMode == MeasureSpec.AT_MOST) {
            heightSize = MAX_LINE_LENGTH;
        }
        ...
        switch (mStep % 4) {
            case 0:
                for (int i = 0; i < mColors.length; i++) {
                    ...
                    drawCRLC(canvas, mWidth / 2 - mEntireLineLength / 2.2f, mHeight / 2 - mLineLength, mWidth / 2 - mEntireLineLength / 2.2f, mHeight / 2 + mEntireLineLength,
            ...
            case 3:
                for (int i = 0; i < mColors.length; i++) {
                    mPaint.setColor(mColors[i]);
                    drawLC(canvas, mWidth / 2 - mEntireLineLength / 2.2f, mHeight / 2 + mEntireLineLength, mWidth / 2 - mEntireLineLength / 2.2f, mHeight / 2 + mLineLength,
                            mPaint, mCanvasAngle + i * 90);
                }
                break;
            default:
    ...
    private void startCRLCAnim() {
        ...
        canvasRotateAnim.addUpdateListener(animation -> mCanvasAngle = (int) animation.getAnimatedValue());
