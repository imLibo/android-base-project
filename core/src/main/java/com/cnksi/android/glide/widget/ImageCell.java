...
public class ImageCell extends AppCompatImageView {
                ...
                .into(new DrawableTarget(this) {
                    ...
                    public void onResourceReady(@NonNull Drawable resource, @Nullable Transition<? super Drawable> transition) {
                        ...
                        if (resource instanceof GifDrawable) {
                            isGif = true;
                            if (!loadGif) {
                                drawable = new BitmapDrawable(getResources(), ((GifDrawable) resource).getFirstFrame());
                            }
                        } else {
                            isGif = false;
                        }
                    ...
                    public void onLoadFailed(@Nullable Drawable errorDrawable) {
                        isGif = Utils.isGif(String.valueOf(url));
                        initCornerIcon();
    ...
    public Drawable getGifDrawable() {
        if (gifDrawable == null) {
            gifDrawable = Utils.getTextDrawable(getContext().getApplicationContext(), 24, 14, 2, "GIF", 11, R.color.glide_nine_image_text_background_color);
        }
        return gifDrawable;
    }
    ...
    public Drawable getLongDrawable() {
        if (longDrawable == null) {
            longDrawable = Utils.getTextDrawable(getContext().getApplicationContext(), 25, 14, 2, "长图", 10, R.color.glide_nine_image_text_background_color);
        }
        return longDrawable;
    }
    ...
    public void draw(Canvas canvas) {
        ...
        if (!TextUtils.isEmpty(imageData.text)) {
            canvas.drawColor(getResources().getColor(R.color.glide_nine_image_text_background_color));
    ...
    private class DrawableTarget extends DrawableImageViewTarget {
        ...
        private void setBackground() {
            if (radius == 0) {
                getView().setBackgroundResource(R.drawable.glide_drawable_image_bg_0dp);
            } else if (radius == 5) {
                getView().setBackgroundResource(R.drawable.glide_drawable_image_bg_5dp);
            } else {
                GradientDrawable drawable = (GradientDrawable) getResources().getDrawable(R.drawable.glide_drawable_image_bg_0dp);
